...
def encoder():
    ...
    x = Conv2D(capacity, kernel_size=3, strides=2, padding="same", activation='relu', name='kok')(images)  # 256X256
    x = Conv2D(2 * capacity, kernel_size=3, strides=2, padding="same", activation='relu', name='vvvvvvv')(x)  # 128X128
    x = Conv2D(4 * capacity, kernel_size=3, strides=2, padding="same", activation='relu')(x)  # 64X64
    x = Conv2D(8 * capacity, kernel_size=3, strides=2, padding="same", activation='relu')(x)  # 32X32
    x = Conv2D(16 * capacity, kernel_size=3, strides=2, padding="same", activation='relu')(x)  # 16X16
    x = Conv2D(32 * capacity, kernel_size=3, strides=2, padding="same", activation='relu')(x)  # 8X8
...
def decoder():
    ...
    x = Conv2DTranspose(32 * capacity, kernel_size=3, strides=2, padding='same', activation='relu')(x)
