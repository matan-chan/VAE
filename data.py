...
def read_img(image_url):
    success = True
    try:
        img_bytes = requests.get(image_url, timeout=1).content
...
def extract_relevant_data_from_files():
    ...
    # data_frame = data_frame.loc[(data_frame['status'] == 'success')]

    return data_frame['url']


def download_images(start=0):
    ...
    for i, url in enumerate(urls):
        try:
            if i > 17166:
                ...
                if success and image.all():
                    im_rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
                    im_rgb = np.array(preprocess_image(im_rgb, False))
                    cv2.imwrite(f'data/images/{i + start}.jpg', im_rgb)
        except:
            continue
